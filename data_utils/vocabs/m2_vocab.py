    ...
    def encode_answer(self, answer: List[str], ocr_tokens: List[str]) -> torch.Tensor:
        ...
        for w in answer:
            if w in self.stoi:
                ...
                norm_ans.append(self.unk_token)

        ids = [self.stoi.get(w, self.unk_idx) for w in norm_ans]

        vec = torch.full((self.max_answer_length,), self.padding_idx, dtype=torch.long)
        for i, idx in enumerate([self.bos_idx] + ids[: self.max_answer_length - 2] + [self.eos_idx]):
            vec[i] = idx
        return vec
