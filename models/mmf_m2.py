...
class MultiHeadAttention(Module):
  ...
  def forward(self, queries, keys, values, attention_mask=None, attention_weights=None):
      if self.can_be_stateful and self._is_stateful:
          # Adding Memory Slots into keys and values
...
class PositionWiseFeedForward(nn.Module):
    ...
    def forward(self, input): # input: [seq_len, features]
        if self.identity_map_reordering:
...
class BeamSearch(object):
    ...
    def _expand_state(self, selected_beam, cur_beam_size):
        def fn(s):
            ...
            s = torch.gather(s.view(*([self.b_s, cur_beam_size] + shape[1:])), dim=1,
...
class Transformer(CaptioningModel):
    ...
    def forward(self, items):
        images = items.region_features             # [bs, 36, 2048]
        seq = items.answer_tokens                  # [bs, T]
        enc_output, mask_enc = self.encoder(images)
        dec_output = self.decoder(seq, enc_output, mask_enc)
        return {"scores": dec_output}
